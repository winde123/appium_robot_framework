...
- 2026-09-06: Dev lead reviewed and integrated the combined patch into `main` as `f653a02` and pushed to origin/main; full device-free battery green on the integrated checkout (164 unit tests, doctests, 66/66 dryruns both forks, strict parity 0/0). Template fidelity spot-checked in review — every test name, count, case mapping and quirk preserved (including the original IHL-header reuse in ltvp "other schools"); the non-uniform third appt_services case correctly kept its explicit body. Worker worktrees/branches removed. Live-device acceptance remains open.
